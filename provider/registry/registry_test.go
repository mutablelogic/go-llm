...
func (c *registryModelClient) Self() llm.Client {
	return c
}
...
func (c *registryModelClient) ListModels(context.Context) ([]schema.Model, error) {
	if c.err != nil {
		return nil, c.err
	}
...
func (c *registryModelClient) GetModel(_ context.Context, name string) (*schema.Model, error) {
	if c.getErr != nil {
		return nil, c.getErr
	}
	if c.err != nil {
...
func TestRegistryGetModelsInvalidPattern(t *testing.T) {
	...
	r := New()
	r.providers["ollama"] = provider{client: NewCachedClient(&registryModelClient{name: "ollama"}, 0)}
...
func TestRegistryGetModelsIncludeExclude(t *testing.T) {
	...
	r := New()
	r.providers["ollama"] = provider{client: NewCachedClient(&registryModelClient{
		name: "ollama",
		models: []schema.Model{
			{Name: "llama3.2"},
			{Name: "mistral-small"},
...
func TestRegistryGetModelsOverwritesOwnedBy(t *testing.T) {
	...
	r := New()
	r.providers["proxy"] = provider{client: NewCachedClient(&registryModelClient{
		name:   "proxy",
...
func TestRegistryGetModelsWrapsProviderError(t *testing.T) {
	...
	r := New()
	r.providers["mistral-primary"] = provider{client: NewCachedClient(&registryModelClient{
		name: "mistral",
...
func TestRegistryGetModelWrapsProviderError(t *testing.T) {
	...
	r := New()
	r.providers["google-prod"] = provider{client: NewCachedClient(&registryModelClient{
		name: "google",
