...
func GeneratorMetaFromValues(values url.Values) GeneratorMeta {
	...
	if v := strings.TrimSpace(values.Get("provider")); v != "" {
		meta.Provider = types.Ptr(v)
	}
	if v := strings.TrimSpace(values.Get("model")); v != "" {
		meta.Model = types.Ptr(v)
	}
...
// MergeFrom returns a new GeneratorMeta using the receiver as the base and
// filling in any non-blank fields from override.
func (g GeneratorMeta) MergeFrom(override GeneratorMeta) GeneratorMeta {
	return MergeGeneratorMeta(override, g)
}
