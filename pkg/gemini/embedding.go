...
func (c *Client) Embedding(ctx context.Context, model schema.Model, text string, opts ...opt.Opt) ([]float64, error) {
	...
	if err := c.DoWithContext(ctx, httpReq, &response, client.OptPath("models", model.Name+":embedContent")); err != nil {
		return nil, err
	}
...
func (c *Client) BatchEmbedding(ctx context.Context, model schema.Model, texts []string, opts ...opt.Opt) ([][]float64, error) {
	...
	for _, text := range texts {
		req, err := c.embedContentRequest(model.Name, text, opts...)
		if err != nil {
			return nil, err
		}
	...
	if err := c.DoWithContext(ctx, httpReq, &response, client.OptPath("models", model.Name+":batchEmbedContents")); err != nil {
		return nil, err
	}
