...
func WithTemperature(value float64) opt.Opt {
	if value < 0 || value > 1 {
		return opt.Error(schema.ErrBadParameter.With("temperature must be between 0.0 and 1.0"))
	}
...
func WithThinking(budgetTokens uint) opt.Opt {
	if budgetTokens < 1024 {
		return opt.Error(schema.ErrBadParameter.With("thinking budget must be at least 1024 tokens"))
	}
...
func WithOutputConfig(value string) opt.Opt {
	if value != "low" && value != "medium" && value != "high" {
		return opt.Error(schema.ErrBadParameter.With("output_config must be 'low', 'medium', or 'high'"))
	}
...
func WithToolChoice(name string) opt.Opt {
	if name == "" {
		return opt.Error(schema.ErrBadParameter.With("tool name is required"))
	}
	return opt.WithOpts(
