...
///////////////////////////////////////////////////////////////////////////////
// WITH TOOL (direct tool definition)

func Test_opt_withtool_001(t *testing.T) {
	// Test WithTool adds a tool to the request
	assert := assert.New(t)
	...
	msg := &schema.Message{Role: "user", Content: []schema.ContentBlock{{Text: strPtr("Hi")}}}
	session := schema.Session{msg}
	o, err := opt.Apply(WithTool(toolDef))
	assert.NoError(err)
	...
	// Verify the tool JSON round-trips
	var decoded map[string]any
	assert.NoError(json.Unmarshal(req.Tools[0], &decoded))
	assert.Equal("get_weather", decoded["name"])
	assert.Equal("Get weather for a city", decoded["description"])
	assert.NotNil(decoded["input_schema"])
}

func Test_opt_withtool_002(t *testing.T) {
	// Test multiple WithTool calls append tools
	assert := assert.New(t)

	tool1 := schema.ToolDefinition{
		Name:        "get_weather",
		Description: "Get weather",
		InputSchema: &jsonschema.Schema{Type: "object"},
	}
	tool2 := schema.ToolDefinition{
		Name:        "search_web",
	...
	msg := &schema.Message{Role: "user", Content: []schema.ContentBlock{{Text: strPtr("Hi")}}}
	session := schema.Session{msg}
	o, err := opt.Apply(WithTool(tool1), WithTool(tool2))
	assert.NoError(err)
...
func Test_opt_withtool_003(t *testing.T) {
	// Test WithTool with empty name fails
	toolDef := schema.ToolDefinition{
		Name:        "",
		InputSchema: &jsonschema.Schema{Type: "object"},
	}
	_, err := opt.Apply(WithTool(toolDef))
	assert.Error(t, err)
}

func Test_opt_withtool_004(t *testing.T) {
	// Test WithTool with nil schema fails
	toolDef := schema.ToolDefinition{
		Name:        "test_tool",
		InputSchema: nil,
	}
