...
// HasNotifications returns true if the server capabilities indicate that
// out-of-band notifications may be sent (listChanged on tools/prompts/resources,
// or logging support).
func (r ResponseInitialize) HasNotifications() bool {
	if r.Capabilities.Logging != nil {
		return true
	}
	for _, cap := range []map[string]any{
		r.Capabilities.Tools,
		r.Capabilities.Prompts,
		r.Capabilities.Resources,
	} {
		if v, ok := cap["listChanged"]; ok {
			if b, ok := v.(bool); ok && b {
				return true
			}
		}
...
// PromptArgument describes an argument that a prompt accepts
...
// PromptMessage is a message returned by prompts/get
type PromptMessage struct {
	Role    string   `json:"role"` // "user" or "assistant"
	Content *Content `json:"content"`
}

// RequestGetPrompt is the params for prompts/get
type RequestGetPrompt struct {
	Name      string            `json:"name"`
	Arguments map[string]string `json:"arguments,omitempty"`
}

// ResponseGetPrompt is the result for prompts/get
...
const (
	...
	MessageTypeGetPrompt     = "prompts/get"
