...
func Test_timespec_002(t *testing.T) {
	...
	_, err := schema.NewTimeSpec(past, nil)
	assert.Error(err)
}
...
func Test_timespec_004(t *testing.T) {
	...
	assert := assert.New(t)
	_, err := schema.NewTimeSpec("2020-01-01T00:00:00Z", nil)
	assert.Error(err)
}
...
func Test_timespec_005(t *testing.T) {
	...
	assert := assert.New(t)
	ts, err := schema.NewTimeSpec("0 9 * * 1-5", nil)
	assert.NoError(err)
...
func Test_timespec_006(t *testing.T) {
	...
	assert := assert.New(t)
	ts, err := schema.NewTimeSpec("*/15 * * * *", nil)
	assert.NoError(err)
...
func Test_timespec_008(t *testing.T) {
	// step of zero: error
	assert := assert.New(t)
	_, err := schema.NewTimeSpec("*/0 * * * *", nil)
	assert.Error(err)
}
...
func Test_timespec_009(t *testing.T) {
	...
	assert := assert.New(t)
	_, err := schema.NewTimeSpec("60 * * * *", nil)
	assert.Error(err)
}
...
func Test_timespec_010(t *testing.T) {
	...
	assert := assert.New(t)
	_, err := schema.NewTimeSpec("0 0 31 2 *", nil)
	assert.Error(err)
}
...
func Test_timespec_015(t *testing.T) {
	...
	assert := assert.New(t)
	ts, err := schema.NewTimeSpec("0 0 1 6 *", nil)
	assert.NoError(err)
	got := ts.Next(testFrom)
...
func Test_timespec_016(t *testing.T) {
	...
	assert.NoError(err)
	ts, err := schema.NewTimeSpec("0 12 * * *", nyLoc)
	assert.NoError(err)
	got := ts.Next(testFrom)
...
func Test_timespec_017(t *testing.T) {
	...
	assert := assert.New(t)
	yr := 2020
	ts := schema.TimeSpec{Year: &yr, Month: []int{1}, Day: []int{1}, Hour: []int{0}, Minute: []int{0}}
	got := ts.Next(testFrom)
...
func Test_timespec_021(t *testing.T) {
	...
	assert := assert.New(t)
	ts, err := schema.NewTimeSpec("0 0 31 * *", nil)
	assert.NoError(err)
...
func Test_timespec_022(t *testing.T) {
	...
	assert := assert.New(t)
	ts, err := schema.NewTimeSpec("0 0 30,31 * *", nil)
	assert.NoError(err)
...
func Test_timespec_023(t *testing.T) {
	...
	assert := assert.New(t)
	ts, err := schema.NewTimeSpec("0 9 * * *", nil)
	assert.NoError(err)
	s := ts.String()
	assert.Equal("0 9 * * *", s)
}
...
func Test_timespec_037(t *testing.T) {
	...
	nyLoc, err := time.LoadLocation("America/New_York")
	assert.NoError(err)
	ts, err := schema.NewTimeSpec("0 9 * * 1-5", nyLoc)
	assert.NoError(err)
	data, err := ts.MarshalJSON()
	assert.NoError(err)
...
func Test_timespec_042(t *testing.T) {
	...
	assert := assert.New(t)
	_, err := schema.NewTimeSpec("* * * * *", time.Local)
	assert.Error(err)
}
